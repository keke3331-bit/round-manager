...
function initDashboard() {
  ...
  function updatePeriodLabel() {
    ...
    if (periodType === 'weekly') {
      ...
      label.textContent = `${toISO(ws)} 〜 ${toISO(we)}`;
      return;
    }
  ...
  function move(dir) {
    ...
    dateInput.value = toISO(currentDate);
    updatePeriodLabel();
    if (cachedData) renderHistory(cachedData, getDates());
  }
  ...
  dateInput.addEventListener('change', e => {
    currentDate = parseISO(e.target.value);
    updatePeriodLabel();
    if (cachedData) renderHistory(cachedData, getDates());
  });
    ...
    <div class="round-card" data-status="${r.status}" data-id="${r.id}">
      ...
      <div class="round-card__body">
        <div class="round-info">
          ...
          <span class="info-value">${r.destination || '-'}</span>
        </div>
        <div class="round-info">
          <span class="info-label">出発</span>
          <span class="info-value">${r.departureTime} → 戻り予定 ${r.expectedReturnTime}</span>
        </div>
        ${r.purpose ? `<div class="round-info"><span class="info-label">目的</span><span class="info-value">${r.purpose}</span></div>` : ''}
        ${r.memberNumber ? `<div class="round-info"><span class="info-label">会員番号</span><span class="info-value">${r.memberNumber}</span></div>` : ''}
...
async function handleAction(action, roundId) {
  const now = getNow();
  const updates = {};
  ...
  if (action === 'arrive') {
    ...
    updates.status              = 'departed_customer';
    updates.departedCustomerAt  = now;
  } else if (action === 'base_arrived') {
    const notesEl = document.getElementById(`notes-${roundId}`);
    updates.status        = 'completed';
    updates.baseArrivedAt = now;
...
function renderHistory(data, dates) {
  ...
  tbody.innerHTML = '';
  if (!rows.length) {
    tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:#9CA3AF;padding:20px">データがありません</td></tr>';
    return;
  }
...
function setDefaultTimes() {
  ...
  const depH = document.getElementById('dep-hour');
  const depM = document.getElementById('dep-min');
  if (depH) depH.value = h;
  if (depM) depM.value = m;
  ...
  const retH = document.getElementById('ret-hour');
  const retM = document.getElementById('ret-min');
  if (retH) retH.value = rh;
  if (retM) retM.value = rm;
}
...
function initInput() {
  ...
  buildTimeOptions('dep-hour', true);
  buildTimeOptions('dep-min',  false);
  buildTimeOptions('ret-hour', true);
  buildTimeOptions('ret-min',  false);
  setDefaultTimes();
  ...
  form.addEventListener('submit', async e => {
    ...
    if (memberVal && memberVal.length !== 8) {
      memberError.textContent = '会員番号は8桁で入力してください';
      return;
    }

    const depH = document.getElementById('dep-hour').value;
    const depM = document.getElementById('dep-min').value;
    const retH = document.getElementById('ret-hour').value;
    const retM = document.getElementById('ret-min').value;

    const roundData = {
      planner,
      purpose:             document.getElementById('purpose').value.trim(),
      departureTime:       `${depH}:${depM}`,
      expectedReturnTime:  `${retH}:${retM}`,
